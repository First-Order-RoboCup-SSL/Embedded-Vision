...
# ----------------------------
# UI: HSV sliders (OpenCV Hue 0..179)
# ----------------------------
def nothing(x): pass
...
# Two hue ranges to handle red wrap-around: [H1L..H1U] OR [H2L..H2U]
...
cv2.createTrackbar("Morph k", "Controls", 0,   15,  nothing)  # 0 = off, else odd kernel size auto
...
while True:
    ...
    # Make sure lows <= highs (simple clamp)
    ...
    # Optional morphology
    if mk > 0:
        k = mk if mk % 2 == 1 else mk + 1  # enforce odd
    ...
    # Build side-by-side view (resize mask to 3-channel for display)
    mask_bgr = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    vis = np.hstack([frame, mask_bgr, masked])

    # Overlay current thresholds
